...
trading_calendars = [['TOK', 'Japan/Tokyo', ql.Japan(), []],
                     ...
                     ['HKG', 'Asia/Hong Kong', ql.HongKong(), []],
                     ['SYD', 'Sydney/Australia', ql.Australia(), []],
                     ['SGX', 'Asia/Singapore', ql.Australia(), []],
                     ['PAR', 'Europe/Paris', ql.France(), []],
                     ]
            ...
            for wkflow_key, wkflow_value in strat_value['workflows'].items():
                ...
                for wkflow_phase_key, wkflow_phase_value in wkflow_value['workphases'].items():
                    ...
                    for task_key, task_value in wkflow_phase_value['tasks'].items():
                        ...
                        if 'requires' in task_value:
                            requirements = task_value['requires']
                            if 'mkt_data' in requirements:
                                if requirements['mkt_data']:
                                    for mkt_data_key, market_data_value in requirements['mkt_data'].items():
                                        data_point = find_mkt_data_point(mkt_data_key, market_data_value['path'])
                                        if data_point is not None:
                                            task_node.requires.connect(data_point)
                                market_list = []
                                if 'mkt' in requirements:
                                    market_list.append(requirements['mkt'])
                                if 'mkts' in requirements:
                                    for m in requirements['mkts']:
                                        market_list.append(m)
                                for m in market_list:
                                    required_mkt_node = market_nodes[m]
                                    task_node.requires.connect(required_mkt_node)

                        if 'produces' in task_value:
                            products = task_value['produces']
                            if products:
                                market_list = []
                                if 'mkt' in products:
                                    market_list.append(products['mkt'])
                                if 'mkts' in products:
                                    for m in products['mkts']:
                                        market_list.append(m)
                                for m in market_list:
                                    mkt_node = sakuragraphmodel.Market(name=m).save()
                                    market_nodes[m] = mkt_node
                                    task_node.produces.connect(mkt_node)
                                ...
                                for capture_item in capt_list:
                                    ...
                                    for idx, trading_day in enumerate(trading_days):
                                        if trading_day.__class__.__name__ == 'BizDay':
                                            this_revision = sakuragraphmodel.Revision(version=idx,
                                                                                      timestamp=datetime.datetime.now(),
                                                                                      comment="added by auto loader",
                                                                                      userId="loader").save()
    ...
    for idx, d in enumerate(series):
        ...
        for trading_calendar in trading_calendars:
            ...
            if is_weekend:
                continue
            if idx == 0:
                trading_center_node = sakuragraphmodel.TradingCenter(code=trading_calendar[0],
                                                                     tz=trading_calendar[1]).save()
